...
impl SyscallStats {
    ...
    pub fn report(&mut self, syscall_id: u32) {
        *self.syscalls.entry(syscall_id).or_insert(0) += 1;
    }

    pub fn show(&self) {
        let mut syscalls: Vec<_> = self.syscalls.iter().map(|(k, v)| (*k, *v)).collect();
        syscalls.sort_unstable_by_key(|syscall| std::cmp::Reverse(syscall.1));

        for (syscall_id, count) in syscalls.into_iter() {
            let syscall_name = match syscall_name_from_id(syscall_id) {
                Some(name) => Cow::from(name),
                None       => Cow::from(format!("Unknown {}", syscall_id)),
            };

            println!("{:16} | {:>3}", syscall_name, count);
        }
    }
}

fn syscall_name_from_id(id: u32) -> Option<&'static str> {
    match id {
        0   => Some("read"),
        1   => Some("write"),
        2   => Some("open"),
        ...
        172 => Some("iopl"),
        173 => Some("ioperm"),
        174 => Some("create_modul"),
        175 => Some("init_module"),
        176 => Some("delete_module"),
        177 => Some("get_kernel_sym"),
        178 => Some("query_modul"),
        179 => Some("quotactl"),
        180 => Some("nfsservct"),
        181 => Some("getpms"),
        182 => Some("putpms"),
        ...
        205 => Some("set_thread_are"),
        206 => Some("io_setup"),
        207 => Some("io_destroy"),
        208 => Some("io_getevents"),
        209 => Some("io_submit"),
        210 => Some("io_cancel"),
        211 => Some("get_thread_are"),
        212 => Some("lookup_dcookie"),
        213 => Some("epoll_create"),
        214 => Some("epoll_ctl_ol"),
        215 => Some("epoll_wait_ol"),
